...
p.Icon, p.Title, 0, p.Folder, "Window", true, p.IconThemed)
            ...
            if not readfile or not getcustomasset then return end

function p:SetBackgroundImage(FileName, Link)
                -- Auto-download if file doesn't exist
                if not pcall(function() readfile(FileName) end) then
                    writefile(FileName, game:HttpGet(Link))
                end
                repeat task.wait() until pcall(function() readfile(FileName) end)

                local asset = getcustomasset(FileName)
                local ext = string.match(FileName:lower(), "%.([^%.]+)$")
                local isVideo = ext == "mp4" or ext == "webm" or ext == "mov"
                local isGif = ext == "gif"

                -- Clear previous UI children
                self.UIElements.Main.Background:ClearAllChildren()
                ...
                if isVideo then
                    ...
                    video.Parent = self.UIElements.Main.Background
                else
                    ...
                    image.Image = asset
                    image.Parent = self.UIElements.Main.Background
                end
end
